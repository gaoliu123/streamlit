...
def load_financial_data() -> dict:
    ...
    单位：万元（原始单位为元，已换算）。
    注：2023年部分费用数据为基于报告上下文的估算值，已在注释中标注。
    """
    ...
    annual_pnl = pd.DataFrame({
        ...
        "营业成本(万元)":       [220340.00, 264094.04, 320449.39],  # 2023为估算
        "研发费用(万元)":       [26241.48,  31470.81,  42557.74],
        "销售费用(万元)":       [13000.00,  14808.78,  15355.69],   # 2023为估算
        "管理费用(万元)":       [11000.00,  15413.53,  18250.83],   # 2023为估算
    ...
    product_revenue = pd.DataFrame({
        ...
        "2024年毛利率(%)":[38.62,                 16.38,                None,       None],  # None = 未披露
    ...
    # ── 十一、利润瀑布数据（FY2025）──
    waterfall_data = {
        "节点":   ["营业收入", "营业成本",   "毛利润",   "销售费用",
                   "管理费用", "研发费用",   "财务收益", "其他收益净额", "归母净利润"],
        "金额":   [451048.44, -320449.39,   130599.05, -15355.69,
                   -18250.83, -42557.74,    657.19,     3500.00,        50500.02],
...
def render_tab_product(data: dict) -> None:
    ...
    说明汇川在工控领域的产品溢价能力持续增强。西门子DI在PLC、伺服、变频器等核心品类
    面临的价格竞争压力将持续加剧，尤其是汇川PLC在中国市场份额5.7%仍处于快速爬坡阶段。
    </div>
    <div class="insight-box green">
    ✅ <b>新能源汽车动力系统收入203亿元，同比+26.39%</b>，已占总收入45.06%，
    成为与工业自动化并驾齐驱的第二增长极。西门子DI暂无直接竞争，
    但需关注汇川借助新能源汽车客户关系向工厂自动化渗透的协同效应。
    </div>
    <div class="insight-box teal">
    ⚡ <b>新兴产业（机器人+数字能源）收入17.95亿元，同比+15.80%</b>，
    人形机器人、工业机器人、储能PCS三条赛道均在快速布局，
    西门子DI需关注其在机器人控制器领域对Motion Control的潜在冲击。
    </div>
    """, unsafe_allow_html=True)
...
def render_tab_market(data: dict) -> None:
    ...
    with col1:
        ...
        fig_reg.update_layout(
            **base_layout(title="地区收入（FY2025，亿元）", height=300, showlegend=False)
        )
    ...
    with col2:
        ...
        fig_reg_g.update_layout(
            **base_layout(title="地区增速对比（FY2025 vs FY2024）",
    ...
    with col4:
        overseas_rev    = [20393.34, 26488.92]
        overseas_growth = [17.22,    29.89]
        fig_overseas = go.Figure(go.Bar(
            x=["2024年", "2025年"],
...
- 筹划赴港交所上市（H股）
    ...
    说明汇川国际化战略正在加速落地。重点行业包括纺织、锂电、光伏、注塑机等，
    这些恰好是西门子DI在东南亚、中东的传统优势市场。
    </div>
    <div class="insight-box red">
    ⚠️ <b>境外收入占比仅5.87%，但增速快、战略意图明确</b>。
...
def render_tab_rd(data: dict) -> None:
    ...
    with col2:
        ...
        fig_exp_g.update_layout(
            **base_layout(title="费用增速 vs 营收增速（%）", height=380, showlegend=False)
        )
...
- 新能源汽车800V高压平台
    ...
    fig_wf.update_layout(
        **base_layout(title="FY2025 利润瀑布图（万元）", height=470, showlegend=False)
    )
    ...
    <div class="insight-box green">
    ✅ <b>销售费用增速仅3.69%</b>，远低于营收增速21.77%，
    说明汇川的营销效率在持续提升，渠道网络趋于成熟，
    单位营收所需销售投入在下降，品牌效应正在显现。
    </div>
    <div class="insight-box teal">
...
- **注意**：2023年部分费用数据为估算值（财报未单独披露），已在代码注释中标注
